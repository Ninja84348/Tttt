    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"Please specify a valid number and unit of time (hours/days) script by OWNER- @Ninja2399..."
        else:
            response = "Usage: /genkey <amount> <hours/days>"
    else:
        response = f"ONLY OWNER CAN USE💀OWNER OWNER- @Ninja2399..."
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅Key redeemed successfully! Access granted until: {users[user_id]} OWNER- @Ninja2399..."
        else:
            response = f"Invalid or expired key buy from OWNER- @Ninja2399..."
    else:
        response = f"Usage: /redeem <key> if you don't  have  buy from  @Ninja2399..."
...
async def allusers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        if users:
            ...
            response = f"No data found OWNER- @Ninja2399 ..."
    else:
        response = f"ONLY OWNER CAN USE.OWNER- @Ninja2399..."
    await update.message.reply_text(response)
